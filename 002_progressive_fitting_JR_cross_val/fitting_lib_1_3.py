...
#                     else: leakage = False
        ...
        if self.print_graph:
            ...
            plt.ylabel("$\\Omega$ $[Ab/struct]$",fontsize=25)
            ...
            plt.ylabel("residuals $[Ab/struct]$",fontsize=25)
            plt.xlabel("time [s]",fontsize=25)
            plt.ylim(-1,1)
            ...
            labels = ["$s_{" + str(int(i)) + \
                      "}$: $\\omega_{"+str(int(i))+"}$ = " + str(int(self.occupancy_key[0][i]))+ \
            ...
            ax.stackplot(range(0, len(self.run_data) + 1), probabilityVectors.T[end_rankings], colors=cmap.colors[end_rankings], labels=labels)
            ...
            weighted_labels = ["$s_{" + str(int(i)) +  "}$: $\\omega_{" + str(int(i)) + "}p^{t_{final}}_{" + str(int(i)) + "}$ = " + str( np.round(probabilityVectors[-1][i] * self.occupancy_key[0][i], decimals=3)) for i in end_rankings_weighted[0:state_printout_limit]]
            # import pdb; pdb.set_trace()
            ax.stackplot(range(len(weighted_ps[0])), occupancies[end_rankings_weighted], colors=cmap.colors[end_rankings_weighted],labels=weighted_labels)
            # import pdb; pdb.set_trace()
            ax.legend(loc='upper left',fontsize=15)
            plt.scatter(range(len(actual_run)),actual_run, s = 30, c = "k")
            plt.ylabel("occupancy \n $p_{i}\\omega_{i} \ [N_{Ab}/N_{struct}]$",fontsize=25)
            plt.ylim((0, 2.0))
            plt.xlabel("t (sec)",fontsize=25)
            plt.xlim((0, len(self.run_data) + 1))
            plt.tight_layout()
